#!/usr/bin/env python3
"""
download_pdbredo_legacy.py

Parallel download of pdb-redo legacy DSSP files with tqdm.

Usage:
    python download_pdbredo_legacy.py input.txt --outdir legacy_dssp --jobs 6
...
def parse_args():
    p = argparse.ArgumentParser(description="Parallel download of pdb-redo legacy DSSP files with tqdm")
    p.add_argument("input", help="Input whitespace-delimited CSV with PDBchain column")
    p.add_argument("--outdir", default="legacy_dssp", help="Output directory (default: legacy_dssp)")
    ...
    with ThreadPoolExecutor(max_workers=args.jobs) as ex:
        ...
        for fut in as_completed(futures):
            ...
            try:
                ...
                res = {"pdbid": pid, "url": f"https://pdb-redo.eu/dssp/db/{pid}/legacy", "http_code": None, "filepath": None, "filesize": 0, "error": str(e)}
            results.append(res)
            main_bar.update(1)
    main_bar.close()
