...
python scripts/cluster_dendrogram_biclustering.py \
...
Z_clusters = linkage(centroids, method='ward', optimal_ordering=True)
Z_cols = linkage(ct_w_hm.T.values, method='ward', optimal_ordering=True)
col_order = leaves_list(Z_cols)[::-1]  # reverse so diagonal runs top-left → bottom-right
ct_w_hm = ct_w_hm.iloc[:, col_order]
ct_counts_hm = ct_counts_hm.iloc[:, col_order]
xlabels = [xlabels[i] for i in col_order]
...
Z_clusters_core = linkage(centroids_core, method='ward', optimal_ordering=True)
Z_cols_core = linkage(ct_w_core_hm.T.values, method='ward', optimal_ordering=True)
col_order_core = leaves_list(Z_cols_core)[::-1]
ct_w_core_hm = ct_w_core_hm.iloc[:, col_order_core]
ct_counts_core_hm = ct_counts_core_hm.iloc[:, col_order_core]
xlabels_core = [xlabels_core[i] for i in col_order_core]
